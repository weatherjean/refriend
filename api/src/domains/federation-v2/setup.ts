...
federation
        ...
        if (att instanceof Link && (object instanceof Page || object instanceof Article)) {
          ...
          if (linkHref) {
            ...
            try {
              ...
              if (linkPreview) await db.updatePostLinkPreview(post.id, linkPreview);
            } catch { /* ignore */ }
  ...
  .on(Announce, async (ctx, announce) => {
    ...
    if (objectUri.includes('/activities/delete')) {
      try {
        ...
        if (typeStr === 'Delete') {
          ...
          const post = await db.getPostByUri(deleteObjectUri);
          if (!post) {
    ...
    if (!post) {
      ...
      if (postId) post = await db.getPostById(postId);
    }
