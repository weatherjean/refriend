...
export async function fetchAndStoreNote(
  ...
  try {
    ...
    // Check if this note is itself a reply - only accept if parent exists locally (no backfill)
    let inReplyToId: number | null = null;
    const parentReplyUri = note.replyTargetId?.href;
    if (parentReplyUri) {
      const parentPost = await db.getPostByUri(parentReplyUri);
      if (parentPost) {
        inReplyToId = parentPost.id;
      } else {
        console.log(`[Announce] Discarding reply - parent post not found locally: ${parentReplyUri}`);
        return null;
      }
    }
    ...
    if (note.published) {
      ...
      const minDate = Temporal.Instant.from("2007-01-01T00:00:00Z");
      if (Temporal.Instant.compare(note.published, maxDate) <= 0 &&
          Temporal.Instant.compare(note.published, minDate) >= 0) {
        publishedAt = note.published.toString();
      }
    }
